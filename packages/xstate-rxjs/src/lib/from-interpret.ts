...
type FromInterpretReturn<TMachine extends AnyStateMachine, TInterpreter = InterpreterFrom<TMachine>> = {
...
export function fromInterpret<TMachine extends AnyStateMachine>(
  ...
  const select = <T>(selectorFn: (state: StateFrom<TMachine>) => T) => fromSelector(service, selectorFn);
